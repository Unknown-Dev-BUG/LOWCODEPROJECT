...
describe("useTable Hook", () => {
  ...
  it("should call onInterval once at ticks (no duplicates)", async () => {
    ...
    // advance one interval (100ms) at a time so each tick produces its own
    // render/effect cycle - this mirrors how real timers fire and lets us
    // assert `onInterval` is called exactly once per tick (no duplicates).
    for (let i = 0; i < 9; i++) {
      await act(async () => {
